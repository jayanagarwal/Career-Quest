...
import { ColdEmail } from '@/types/database'
import { Plus, Pencil, Trash2, X, Mail, Send } from 'lucide-react'
...
export default function EmailsPage() {
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            ...
            case 'Sent': return 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-300'
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-muted-foreground">Track cold emails and follow-ups.</p>
                </div>
                <button
                    onClick={() => { setEditingEmail(null); setShowForm(true) }}
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 bg-card border border-dashed border-border rounded-xl">
                    ...
                    <h3 className="text-lg font-medium mb-1">No outreach logged</h3>
                    <p className="text-muted-foreground mb-4">Track your first cold email</p>
                    <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {emails.map((email) => (
                        <div key={email.id} className="group relative bg-card border border-border rounded-xl p-5 hover:border-blue-500/50 hover:shadow-lg transition-all duration-300">
                            <div className="flex justify-between items-start mb-4">
                                <div className="w-10 h-10 rounded-full bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center text-indigo-700 dark:text-indigo-300 font-semibold text-lg">
                                    {email.contact_name.charAt(0)}
                                </div>
                            ...
                            <h3 className="font-semibold text-foreground">{email.contact_name}</h3>
                            <p className="text-sm text-muted-foreground mb-4 truncate">{email.contact_email}</p>

                            <div className="space-y-2 mb-4">
                                <div className="flex items-center text-sm text-muted-foreground">
                                    <span className="w-20 text-xs uppercase tracking-wider opacity-70">Role</span>
                                    <span className="text-foreground">{email.contact_role || '-'}</span>
                                </div>
                                <div className="flex items-center text-sm text-muted-foreground">
                                    <span className="w-20 text-xs uppercase tracking-wider opacity-70">Company</span>
                                    <span className="text-foreground">{email.company || '-'}</span>
                                </div>
                            </div>

                            <div className="pt-4 border-t border-border flex items-center justify-between">
                                <p className="text-xs text-muted-foreground">Ref: <span className="text-foreground font-medium">{email.referral_role || 'General'}</span></p>
                                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => { setEditingEmail(email); setShowForm(true) }} className="p-1.5 text-muted-foreground hover:text-blue-600 transition-colors">
                                        <Pencil className="w-4 h-4" />
                                    </button>
                                    <button onClick={() => handleDelete(email.id)} className="p-1.5 text-muted-foreground hover:text-red-600 transition-colors">
            ...
            {showForm && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="w-full max-w-lg bg-card border border-border rounded-xl shadow-2xl animate-in zoom-in-95 duration-200">
                        <EmailForm
                            email={editingEmail}
                            onSuccess={() => { setShowForm(false); setEditingEmail(null); fetchEmails() }}
...
function EmailForm({ email, onSuccess, onCancel }: { email: ColdEmail | null; onSuccess: () => void; onCancel: () => void }) {
    const [formData, setFormData] = useState({
        contact_name: email?.contact_name || '',
        contact_email: email?.contact_email || '',
        contact_role: email?.contact_role || '',
        company: email?.company || '',
        referral_role: email?.referral_role || '',
    ...
    return (
        <div className="p-6">
            <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold">{email ? 'Edit Log' : 'Log Email'}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="pt-4 flex justify-end gap-3">
                    ...
                    <button type="submit" disabled={loading} className="px-4 py-2 text-sm font-medium bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors shadow-sm">{loading ? 'Saving...' : 'Log Email'}</button>
                </div>
            </form>
        </div>
    )
